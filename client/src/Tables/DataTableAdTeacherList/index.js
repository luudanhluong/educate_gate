...
function DataTable({ table, isSorted, noEndBorder }) {
  const { data: listTeacher, count } = useSelector((state) => state.user.teacher);
  ...
  const handleSelectUsersetSelectUser = (key) => {
    const index = selectUser.some((u) => u._id === listTeacher[key]?._id);
    if (!index) {
      dispatch(setSelectUser([...selectUser, listTeacher[key]]));
    } else {
      dispatch(setSelectUser(selectUser.filter((s) => s._id !== listTeacher[key]?._id)));
    }
  };
